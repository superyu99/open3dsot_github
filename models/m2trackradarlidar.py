...
from models.speed_aware_transformer import SpeedAwareTransformer
...
class M2TRACKRADARLIDAR(base_model.MotionBaseModelRadarLidar):
    def __init__(self, config, **kwargs):
        ...
        # 1：时间戳
        # 1：概率
        self.seg_pointnet = SegPointNet(input_channel=3 + 1 + 1 + (9 if self.box_aware else 0), #此处注意输入维度
        ...
        # 3：其他特征
        # 1：时间戳
        self.mini_pointnet = MiniPointNet(input_channel=3 + 1 + (9 if self.box_aware else 0), ##此处注意输入维度
        ...
        self.radar_pointnet = MiniPointNet(input_channel=11, ##Radar的特征个数 此处注意输入维度
                                          per_point_mlp=[64, 128, 256, 512],
                                          hidden_mlp=[512, 256],
                                          output_size=-1)
        self.query_embed = nn.Embedding(self.num_queries, self.hidden_dim * 2) #！！！1,512
        # speed aware transformer
        self.speed_aware_transformer = SpeedAwareTransformer()
    ...
    def forward(self, input_dict):
        ...
        radar_x = input_dict["radar_points_prev"].transpose(1, 2)
        ...
        mask_points = x[:, :4, :] * pred_cls #取出原始特征，赋上权重，此处注意维度，输入多了3个维度
        ...
        # radar_embed = self.radar_pointnet(radar_x)
        # point_feature = self.speed_aware_transformer(tgt,query_embed,point_feature,radar_embed)
        # point_feature = point_feature.squeeze(1)

        # point_feature = torch.cat([point_feature, radar_embed], dim=1)
        #-----------------------feature cross end----------------------------------
