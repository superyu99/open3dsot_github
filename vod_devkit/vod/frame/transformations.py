...
def transform_pcl_with_all_feature(points: np.ndarray, transform_matrix: np.ndarray):
    """
    This function transforms homogenous points using a transformation matrix
    and retains all features of the point cloud.
    :param points: Points to be transformed.
    :param transform_matrix: Homogenous transformation matrix.
    :return: Transformed homogenous points with all features.
    """
    ...
    # Retain the original features (intensity, etc.) in the output array
    points_all_features = np.hstack((points_new_frame[:,0:3], points[:, 3:]))

    return points_all_features
